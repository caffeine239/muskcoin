...
uint256 hashGenesisBlock("0xf1a909372dfcb9f116db303f9ff8ee550c6ac6ecfe609aab89f1fceb5470e495");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Muskcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Muskcoin Signed Message:\n";
...
{
    // Muskcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Muskcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe1636609eb5d44279176ea7424bb862a04453e0b5b153efc3a9a178d25aa240b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nTime    = 1614405828;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085524056;

        if (fTestNet)
        {
            block.nTime    = 1614405805;
            block.nNonce   = 387036667;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xa7c21ccca318aaaf116798a41b2d6d6b021279b3fccf7a23795a556804d29972"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc2, 0xb2, 0xd1 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MuskcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MuskcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MuskcoinMiner(CWallet *pwallet)
{
    printf("MuskcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("muskcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MuskcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MuskcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MuskcoinMiner, pwallet));
}
